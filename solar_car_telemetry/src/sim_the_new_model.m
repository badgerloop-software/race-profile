function res = sim_the_new_model(args)
    % Utility function to simulate a Simulink model (named 'the_new_model') with
    ...
    %    BatterySOC:
    %       Battery state of charge, default is empty
    %    VehicleSpeed:
    %       Vehicle speed, default is empty
    %    SolarPowerInput:
    %       Solar power input, default is empty
    %    SolarIrradiance:
    %       Solar irradiance, default is empty
    %    RoadGrade:
    %       Road grade, default is empty
    %    WindSpeed:
    %       Wind speed, default is empty
    %
    ...
    arguments
        ...
        args.BatterySOC (1,:) {mustBeNumericOrLogical} = []
        args.VehicleSpeed (1,:) {mustBeNumericOrLogical} = []
        args.SolarPowerInput (1,:) {mustBeNumericOrLogical} = []
        args.SolarIrradiance (1,:) {mustBeNumericOrLogical} = []
        args.RoadGrade (1,:) {mustBeNumericOrLogical} = []
        args.WindSpeed (1,:) {mustBeNumericOrLogical} = []
    end
    
        %% Create the SimulationInput object
        % Note that the name of the model is hard-coded to 'the_new_model'
        si = Simulink.SimulationInput('the_new_model');
        ...
        %% Load the additional inputs into the SimulationInput object
        if ~isempty(args.BatterySOC) || ~isempty(args.VehicleSpeed)
            uStruct.time = [];
            uStruct.signals.dimensions = 6;
            uStruct.signals.values = [
                reshape(args.BatterySOC, [], 1),
                reshape(args.VehicleSpeed, [], 1),
                reshape(args.SolarPowerInput, [], 1),
                reshape(args.SolarIrradiance, [], 1),
                reshape(args.RoadGrade, [], 1),
                reshape(args.WindSpeed, [], 1)
            ];
            si.ExternalInput = uStruct;
        end
        ...
        function locPostStepFcn(simTime)
            so = simulink.compiler.getSimulationOutput('the_new_model');
            res = extractResults(so, prevSimTime);
            stopRequested = feval(args.OutputFcn, simTime, res);
            if stopRequested
                simulink.compiler.stopSimulation('the_new_model');
            end
            prevSimTime = simTime;
        end
